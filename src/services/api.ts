...
export const loginAdmin = async (username: string, password: string) => {
  ...
  if (!creds) throw { response: { status: 500, data: { detail: 'Database error: Credentials not found' } } };
...
export const requestChangeOTPs = async (currentPassword: string, newEmail?: string) => {
  const creds = await getAdminCreds();
  if (!creds) throw { response: { status: 500, data: { detail: 'Database error' } } };
  
  if (currentPassword !== creds.password) {
    throw { response: { status: 401, data: { detail: 'Current password is incorrect.' } } };
  }
  
  const currentOTPRes = await sendOTPEmail(creds.email, 'mock_admin_otp');
